...
public final class Fama implements AutoCloseable {
    ...
    public Fama(double fastLimit, double slowLimit) {
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_FAMA_NEW.invokeExact(fastLimit, slowLimit);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid Fama parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_FAMA_FREE);
    }
