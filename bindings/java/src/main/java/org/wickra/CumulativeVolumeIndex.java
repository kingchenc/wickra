...
/** Streaming CumulativeVolumeIndex indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class CumulativeVolumeIndex implements AutoCloseable {
    ...
    public CumulativeVolumeIndex() {
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_CUMULATIVE_VOLUME_INDEX_NEW.invokeExact();
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid CumulativeVolumeIndex parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_CUMULATIVE_VOLUME_INDEX_FREE);
    }
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_CUMULATIVE_VOLUME_INDEX_RESET.invokeExact(handle());
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
            Reference.reachabilityFence(this);
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("CumulativeVolumeIndex has been closed");
        }
        return handle;
    }
