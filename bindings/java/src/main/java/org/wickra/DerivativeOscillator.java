...
public final class DerivativeOscillator implements AutoCloseable {
    ...
    public double update(double value) {
        try {
            return (double) NativeMethods.WICKRA_DERIVATIVE_OSCILLATOR_UPDATE.invokeExact(handle, value);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] input) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_DERIVATIVE_OSCILLATOR_BATCH.invokeExact(handle, inputSeg, outSeg, (long) n);
