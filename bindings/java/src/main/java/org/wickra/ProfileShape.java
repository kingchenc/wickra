...
/** Streaming ProfileShape indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class ProfileShape implements AutoCloseable {
    ...
    public ProfileShape(int period, int bins) {
        ...
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_PROFILE_SHAPE_NEW.invokeExact((long) period, (long) bins);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid ProfileShape parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_PROFILE_SHAPE_FREE);
    }
    ...
    public double update(double open, double high, double low, double close, double volume, long timestamp) {
        try {
            return (double) NativeMethods.WICKRA_PROFILE_SHAPE_UPDATE.invokeExact(handle, open, high, low, close, volume, timestamp);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, double[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_PROFILE_SHAPE_BATCH.invokeExact(handle, openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_PROFILE_SHAPE_WARMUP_PERIOD.invokeExact(handle);
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_PROFILE_SHAPE_IS_READY.invokeExact(handle);
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_PROFILE_SHAPE_NAME.invokeExact(handle);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_PROFILE_SHAPE_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
