...
public final class TdMovingAverage implements AutoCloseable {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_TD_MOVING_AVERAGE_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("TdMovingAverage has been closed");
        }
        return handle;
    }
