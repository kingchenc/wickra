...
/** Streaming CloseVsOpen indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class CloseVsOpen implements AutoCloseable {
    ...
    public CloseVsOpen() {
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_CLOSE_VS_OPEN_NEW.invokeExact();
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid CloseVsOpen parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_CLOSE_VS_OPEN_FREE);
    }
    ...
    public double update(double open, double high, double low, double close, double volume, long timestamp) {
        try {
            return (double) NativeMethods.WICKRA_CLOSE_VS_OPEN_UPDATE.invokeExact(handle, open, high, low, close, volume, timestamp);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, double[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_CLOSE_VS_OPEN_BATCH.invokeExact(handle, openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_CLOSE_VS_OPEN_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
