...
public final class TimeOfDayReturnProfile implements AutoCloseable {
    ...
    public double[][] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, long[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_TIME_OF_DAY_RETURN_PROFILE_BATCH.invokeExact(handle(), openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, flat, (long) width, (long) n);
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_TIME_OF_DAY_RETURN_PROFILE_WARMUP_PERIOD.invokeExact(handle());
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_TIME_OF_DAY_RETURN_PROFILE_IS_READY.invokeExact(handle());
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_TIME_OF_DAY_RETURN_PROFILE_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("TimeOfDayReturnProfile has been closed");
        }
        return handle;
    }
