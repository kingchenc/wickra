...
/** Streaming ChaikinVolatility indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class ChaikinVolatility implements AutoCloseable {
    ...
    public ChaikinVolatility(int emaPeriod, int rocPeriod) {
        ...
        if (rocPeriod < 0) {
            throw new IllegalArgumentException("rocPeriod must be non-negative");
        }
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_CHAIKIN_VOLATILITY_NEW.invokeExact((long) emaPeriod, (long) rocPeriod);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid ChaikinVolatility parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_CHAIKIN_VOLATILITY_FREE);
    }
    ...
    public double update(double open, double high, double low, double close, double volume, long timestamp) {
        try {
            return (double) NativeMethods.WICKRA_CHAIKIN_VOLATILITY_UPDATE.invokeExact(handle, open, high, low, close, volume, timestamp);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, double[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_CHAIKIN_VOLATILITY_BATCH.invokeExact(handle, openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_CHAIKIN_VOLATILITY_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
