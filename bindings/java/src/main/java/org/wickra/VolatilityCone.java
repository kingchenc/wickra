...
public final class VolatilityCone implements AutoCloseable {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_VOLATILITY_CONE_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("VolatilityCone has been closed");
        }
        return handle;
    }
