...
/** Streaming CupAndHandle indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class CupAndHandle implements AutoCloseable {
    ...
    public CupAndHandle() {
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_CUP_AND_HANDLE_NEW.invokeExact();
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid CupAndHandle parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_CUP_AND_HANDLE_FREE);
    }
    ...
    public double update(double open, double high, double low, double close, double volume, long timestamp) {
        try {
            return (double) NativeMethods.WICKRA_CUP_AND_HANDLE_UPDATE.invokeExact(handle, open, high, low, close, volume, timestamp);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, double[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_CUP_AND_HANDLE_BATCH.invokeExact(handle, openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_CUP_AND_HANDLE_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
