...
/** Streaming ConditionalValueAtRisk indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class ConditionalValueAtRisk implements AutoCloseable {
    ...
    public ConditionalValueAtRisk(int period, double confidence) {
        ...
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_CONDITIONAL_VALUE_AT_RISK_NEW.invokeExact((long) period, confidence);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid ConditionalValueAtRisk parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_CONDITIONAL_VALUE_AT_RISK_FREE);
    }

    /** Push one observation; returns the indicator value (NaN during warmup). */
    public double update(double value) {
        try {
            return (double) NativeMethods.WICKRA_CONDITIONAL_VALUE_AT_RISK_UPDATE.invokeExact(handle, value);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] input) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_CONDITIONAL_VALUE_AT_RISK_BATCH.invokeExact(handle, inputSeg, outSeg, (long) n);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_CONDITIONAL_VALUE_AT_RISK_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
