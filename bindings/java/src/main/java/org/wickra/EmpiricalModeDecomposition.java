...
public final class EmpiricalModeDecomposition implements AutoCloseable {
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_EMPIRICAL_MODE_DECOMPOSITION_WARMUP_PERIOD.invokeExact(handle);
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_EMPIRICAL_MODE_DECOMPOSITION_IS_READY.invokeExact(handle);
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_EMPIRICAL_MODE_DECOMPOSITION_NAME.invokeExact(handle);
