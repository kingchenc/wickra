...
public final class SmoothedHeikinAshi implements AutoCloseable {
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_SMOOTHED_HEIKIN_ASHI_WARMUP_PERIOD.invokeExact(handle());
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_SMOOTHED_HEIKIN_ASHI_IS_READY.invokeExact(handle());
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_SMOOTHED_HEIKIN_ASHI_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("SmoothedHeikinAshi has been closed");
        }
        return handle;
    }
