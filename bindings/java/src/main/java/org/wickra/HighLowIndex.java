...
/** Streaming HighLowIndex indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class HighLowIndex implements AutoCloseable {
    ...
    public HighLowIndex(int period) {
        ...
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_HIGH_LOW_INDEX_NEW.invokeExact((long) period);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid HighLowIndex parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_HIGH_LOW_INDEX_FREE);
    }
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_HIGH_LOW_INDEX_WARMUP_PERIOD.invokeExact(handle());
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_HIGH_LOW_INDEX_IS_READY.invokeExact(handle());
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_HIGH_LOW_INDEX_NAME.invokeExact(handle());
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_HIGH_LOW_INDEX_RESET.invokeExact(handle());
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
            Reference.reachabilityFence(this);
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("HighLowIndex has been closed");
        }
        return handle;
    }
