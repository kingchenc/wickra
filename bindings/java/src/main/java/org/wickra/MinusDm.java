...
/** Streaming MinusDm indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class MinusDm implements AutoCloseable {
    ...
    public MinusDm(int period) {
        ...
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_MINUS_DM_NEW.invokeExact((long) period);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid MinusDm parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_MINUS_DM_FREE);
    }
    ...
    public double update(double open, double high, double low, double close, double volume, long timestamp) {
        try {
            return (double) NativeMethods.WICKRA_MINUS_DM_UPDATE.invokeExact(handle, open, high, low, close, volume, timestamp);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, double[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_MINUS_DM_BATCH.invokeExact(handle, openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_MINUS_DM_WARMUP_PERIOD.invokeExact(handle);
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_MINUS_DM_IS_READY.invokeExact(handle);
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_MINUS_DM_NAME.invokeExact(handle);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_MINUS_DM_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
