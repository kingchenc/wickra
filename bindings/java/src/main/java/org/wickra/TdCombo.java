...
/** Streaming TdCombo indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class TdCombo implements AutoCloseable {
    ...
    public TdCombo(int setupLookback, int setupTarget, int countdownLookback, int countdownTarget) {
        ...
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_TD_COMBO_NEW.invokeExact((long) setupLookback, (long) setupTarget, (long) countdownLookback, (long) countdownTarget);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid TdCombo parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_TD_COMBO_FREE);
    }
    ...
    public double update(double open, double high, double low, double close, double volume, long timestamp) {
        try {
            return (double) NativeMethods.WICKRA_TD_COMBO_UPDATE.invokeExact(handle, open, high, low, close, volume, timestamp);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] open, double[] high, double[] low, double[] close, double[] volume, double[] timestamp) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_TD_COMBO_BATCH.invokeExact(handle, openSeg, highSeg, lowSeg, closeSeg, volumeSeg, timestampSeg, outSeg, (long) n);
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_TD_COMBO_WARMUP_PERIOD.invokeExact(handle);
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_TD_COMBO_IS_READY.invokeExact(handle);
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_TD_COMBO_NAME.invokeExact(handle);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_TD_COMBO_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
