...
/** Streaming CommonSenseRatio indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class CommonSenseRatio implements AutoCloseable {
    ...
    public CommonSenseRatio(int period) {
        ...
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_COMMON_SENSE_RATIO_NEW.invokeExact((long) period);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid CommonSenseRatio parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_COMMON_SENSE_RATIO_FREE);
    }

    /** Push one observation; returns the indicator value (NaN during warmup). */
    public double update(double value) {
        try {
            return (double) NativeMethods.WICKRA_COMMON_SENSE_RATIO_UPDATE.invokeExact(handle, value);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
    ...
    public double[] batch(double[] input) {
        ...
        try (Arena a = Arena.ofConfined()) {
            ...
            NativeMethods.WICKRA_COMMON_SENSE_RATIO_BATCH.invokeExact(handle, inputSeg, outSeg, (long) n);
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_COMMON_SENSE_RATIO_RESET.invokeExact(handle);
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
    }
