...
/** Streaming TickIndex indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class TickIndex implements AutoCloseable {
    ...
    public TickIndex() {
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_TICK_INDEX_NEW.invokeExact();
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid TickIndex parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_TICK_INDEX_FREE);
    }
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_TICK_INDEX_WARMUP_PERIOD.invokeExact(handle());
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_TICK_INDEX_IS_READY.invokeExact(handle());
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_TICK_INDEX_NAME.invokeExact(handle());
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_TICK_INDEX_RESET.invokeExact(handle());
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
            Reference.reachabilityFence(this);
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("TickIndex has been closed");
        }
        return handle;
    }
