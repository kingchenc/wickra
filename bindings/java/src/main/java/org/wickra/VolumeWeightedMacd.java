...
public final class VolumeWeightedMacd implements AutoCloseable {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_VOLUME_WEIGHTED_MACD_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("VolumeWeightedMacd has been closed");
        }
        return handle;
    }
