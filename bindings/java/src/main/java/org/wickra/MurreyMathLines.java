...
public final class MurreyMathLines implements AutoCloseable {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_MURREY_MATH_LINES_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("MurreyMathLines has been closed");
        }
        return handle;
    }
