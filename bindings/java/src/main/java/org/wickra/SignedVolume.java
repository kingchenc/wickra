...
public final class SignedVolume implements AutoCloseable {
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_SIGNED_VOLUME_WARMUP_PERIOD.invokeExact(handle());
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_SIGNED_VOLUME_IS_READY.invokeExact(handle());
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_SIGNED_VOLUME_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("SignedVolume has been closed");
        }
        return handle;
    }
