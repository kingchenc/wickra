...
/** Streaming UpDownVolumeRatio indicator over the Wickra C ABI. Not thread-safe; close when done. */
public final class UpDownVolumeRatio implements AutoCloseable {
    ...
    public UpDownVolumeRatio() {
        MemorySegment h;
        try {
            h = (MemorySegment) NativeMethods.WICKRA_UP_DOWN_VOLUME_RATIO_NEW.invokeExact();
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        }
        if (h.address() == 0L) {
            throw new IllegalArgumentException("invalid UpDownVolumeRatio parameters");
        }
        this.handle = h;
        this.cleanable = WickraNative.register(this, h, NativeMethods.WICKRA_UP_DOWN_VOLUME_RATIO_FREE);
    }
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_UP_DOWN_VOLUME_RATIO_WARMUP_PERIOD.invokeExact(handle());
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_UP_DOWN_VOLUME_RATIO_IS_READY.invokeExact(handle());
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_UP_DOWN_VOLUME_RATIO_NAME.invokeExact(handle());
    ...
    public void reset() {
        try {
            NativeMethods.WICKRA_UP_DOWN_VOLUME_RATIO_RESET.invokeExact(handle());
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
            Reference.reachabilityFence(this);
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("UpDownVolumeRatio has been closed");
        }
        return handle;
    }
