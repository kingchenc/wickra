...
public final class HighLowVolumeNodes implements AutoCloseable {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_HIGH_LOW_VOLUME_NODES_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("HighLowVolumeNodes has been closed");
        }
        return handle;
    }
