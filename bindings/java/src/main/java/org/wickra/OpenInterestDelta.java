...
public final class OpenInterestDelta implements AutoCloseable {
    ...
    public int warmupPeriod() {
        try {
            long n = (long) NativeMethods.WICKRA_OPEN_INTEREST_DELTA_WARMUP_PERIOD.invokeExact(handle());
            return (int) n;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    public boolean isReady() {
        try {
            byte r = (byte) NativeMethods.WICKRA_OPEN_INTEREST_DELTA_IS_READY.invokeExact(handle());
            return r != 0;
        } catch (Throwable t) {
            throw WickraNative.rethrow(t);
        } finally {
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("OpenInterestDelta has been closed");
        }
        return handle;
    }
