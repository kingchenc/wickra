...
public final class ProjectionBands implements AutoCloseable {
    ...
    public String name() {
        try {
            MemorySegment s = (MemorySegment) NativeMethods.WICKRA_PROJECTION_BANDS_NAME.invokeExact(handle());
    ...
    private MemorySegment handle() {
        if (closed) {
            throw new IllegalStateException("ProjectionBands has been closed");
        }
        return handle;
    }
