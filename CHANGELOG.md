...
### Changed
- **GitHub org migrated from `kingchenc` to `wickra-lib`.** All repository
  URLs in badges, manifests, READMEs, license notice, and CHANGELOG
  compare links now point at `https://github.com/wickra-lib/wickra`.
  Maintainer contact email switched to `wickra.lib@gmail.com`
  (SECURITY.md and CODE_OF_CONDUCT.md). Package names on crates.io /
  PyPI / npm are unchanged. GitHub's automatic 301-redirects keep the
  old URLs working, but new references should use the new ones.

### Added
- **`wickra_core::FAMILIES` constant.** Machine-readable
  `&[(&str, &[&str])]` taxonomy that maps every built-in indicator to
  one of 16 families. Comes with two compile-time guards
  (`no_duplicates_across_families`, `total_count_matches_expected`) so
  the table cannot drift silently as new indicators are added.
- **`repo-metadata.toml` + `sync-metadata.yml` workflow** — single
  source of truth for repo identity (org, email, canonical URLs) plus
  an audit workflow that fails CI if any tracked file drifts back to
  a pre-migration value.
- **`bindings/node/package-lock.json` is now committed.** Contributors
  get reproducible npm installs.
- **CycloneDX SBOM per published crate.** Generated by `cargo-cyclonedx`
  during release and attached to the GitHub Release alongside wheels /
  tarballs / `.crate` files.
- **npm `--provenance` attestations** on every npm publish call (main
  `wickra`, six per-platform `wickra-<triple>` sub-packages,
  `wickra-wasm`). Verified-provenance badge appears on npmjs.com.
- **WASM CI tests run on every PR.** The existing 8 `wasm_bindgen_test`
  cases plus 10 new ones (cross-family coverage) now execute under
  `wasm-pack test --node` in CI on every push and pull-request.
- **Three end-to-end strategy examples** under `examples/{rust,python}/`:
  RSI mean-reversion, MACD + ADX trend filter, Bollinger squeeze
  breakout — each with PnL / Sharpe / max-drawdown summary over the
  bundled BTCUSDT datasets.
- **Governance + onboarding files**: `ARCHITECTURE.md`, `ROADMAP.md`,
  `CITATION.cff`, `.github/FUNDING.yml`, `.editorconfig`.
...
[Unreleased]: https://github.com/wickra-lib/wickra/compare/v0.3.0...HEAD
